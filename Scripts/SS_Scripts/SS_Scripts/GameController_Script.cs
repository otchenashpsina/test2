...
public class Coins
{
	public GameObject coinObj;       //Object Prefab
...
{
	//Public Var
	public Coins coinn;                 //make an Object from Class coin
	...
	void Start ()
	{
		StartCoroutine(coinSpawnWaves());   //Start IEnumerator function
	...
	//Coin IEnumerator Coroutine
	IEnumerator coinSpawnWaves()
	{
		yield return new WaitForSeconds(coinn.StartWait);                                                            //Wait for Seconds before start the wave

		//Infinite Loop
		while (true)
		{
			//Spawn Specific number of Objects in 1 wave
			for (int i = 0; i < coinn.Count; i++)
			{
				Vector2 spawnPosition = new Vector2(Random.Range(-spawnValues.x, spawnValues.x), spawnValues.y);        //Random Spawn Position
				Quaternion spawnRotation = Quaternion.identity;                                                         //Default Rotation
				Instantiate(coinn.coinObj, spawnPosition, spawnRotation);                                     //Instantiate Object
				yield return new WaitForSeconds(coinn.SpawnWait);                                                    //Wait for seconds before spawning the next object
			}
			yield return new WaitForSeconds(coinn.WaveWait);                                                         //wait for seconds before the next wave
		}
	}
